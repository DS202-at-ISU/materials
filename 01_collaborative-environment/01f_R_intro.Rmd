...
- use boolean (logical) variables: row filter
...
- What are the number of observations and the number of variables for the `fbi` data set
...
- Read `?mean` and `?sd`, and fix missing value problem

- Pull the number of counts for `homicide` as a vector named `homicide_vec`

- Compute mean and standard deviation for `homicide_vec`

- Create a new column named `ratio` for `fbi` data set with the formula: `ratio = count / population * 10000`
...
Write a function named `compute_mean_sd` for the `fbi` data set with the following requirements:

- This function computes the mean and standard deviation of the number of counts for a certain type of crime

- The input is the crime type

- The output is a one-row data frame with three columns: `type`, `mean`, `sd`

- The computation of mean and standard deviation can handle `NA`s

---
class: inverse
## Your turn 

Use `compute_mean_sd` and get the results for all types of crime

- `for` loop?

- `apply` functions?

- Put together all results with `rbind`. `?rbind`
