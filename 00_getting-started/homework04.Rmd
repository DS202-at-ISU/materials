...
sum(chick_count$n == 12)

chick_count %>% filter(n < 12) %>% 
  pull(n) %>% mean()

complete_id <- chick_count %>% filter(n == 12) %>% pull(Chick)

complete <- ChickWeight %>% 
  filter(Chick %in% complete_id)
...
```{r}
complete %>% 
  ggplot(aes(x = reorder(Diet, weightgain, median, decreasing = TRUE), y = weightgain)) +
  geom_boxplot()
```
